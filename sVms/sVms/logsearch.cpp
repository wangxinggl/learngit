...
void LogSearch::on_btnLogAlmSearch_clicked()
{
	...
	if (g_pDatabase->QuerryAlarmLog(m_lstAlarmLog, m_TimeBegin, m_TimeEnd) <= 0)
		return;
...
int LogSearch::GetCtrlData(int nAlarmConfId, QString sBeginTime, QString sHandleTime, QString sHandleUser)
{
	...
	QString szDevName;
	QString szTmp;
	int nColumn = g_pDatabase->getAlarmConfMsg(nAlarmConfId, devId,alarmSrc,channel,alarmType,alarmLevel);
	...
	for (int i = 0;i<1;i++)
	{
		for (int j = 0;j < nColumn ;j++)
		{
			if (j == 0)
			{
				int nRow = g_pDatabase->getDevInfo(devId, szDevName);
				if (nRow != 0)
				{
...
void LogSearch::on_btnLogDel_clicked()
{
	...
	if (m_lstAlarmLog.size() != 0)
	{
		...
		if (nId != 0)
		{
			g_pDatabase->DeleteAlarmLog(nId);
		}
	}
}
...
void LogSearch::on_btnClear_clicked()
{
	...
	if (m_lstAlarmLog.size() != 0)
	{
		...
		g_pDatabase->DeleteAlarmLog(nId,true);
...
int LogSearch::AddMsg(int nDevId, TabAlarmConf_t* pAlarmConf)
{
	int nCount;
	QString strST;
	QString  strAT;
	...
	CJLNDevice*  devInfo = m_pTreeDev->GetBaseDeviceInfo(-1, nDevId);
	if (devInfo == NULL)
		return 0;
