...
{
	m_layerDrawInfo->setTimeLineRealTime(time);
}
...
{
	m_layerDrawInfo = new LayerDrawInfo;
    initTimeBackWidget();
	...
	m_timeLineWidget->setLayerDrawInfoObject(m_layerDrawInfo);
	m_layerDrawWidget->setLayerDrawInfoObject(m_layerDrawInfo);
	// 初始化默认添加6个图层;
...
{
	connect(m_layerDrawInfo, &LayerDrawInfo::signalUpdateCurrentTime, m_timeHeaderWidget, &TimeHeaderWidget::onUpdateCurrentTime);
