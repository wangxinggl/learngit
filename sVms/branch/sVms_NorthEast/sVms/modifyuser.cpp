...
void modifyUser::on_btnSure_clicked()
{
	...
	else if (!Otherchar(newPassword.toLatin1().data())||!Otherchar(newRePassword.toLatin1().data()))
	{
	...
	DB *db = new DB;
	bool ret = db->CheckUser(name, oldPassword);
	if (!ret)
	{
	...
	{
		ret = db->ModifyUser(name, newPassword);
		if (!ret)
		{
