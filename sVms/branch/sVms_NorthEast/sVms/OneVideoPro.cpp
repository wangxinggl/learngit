...
int OneVideoPro::AddWatch(TreeData_t* pDev, int nPatrolId, BOOL bTipBox /* = FALSE */, int nWndNum /* = 0 */)
{
	QString  strText;
	QString  strName;
	...
	if (m_dspWndinfo.bVS)
	{
		//QMessageBox::warning(NULL, QObject::tr("警告"), QObject::tr("窗口正在轮询视频"));
		//myHelper::ShowMessageBoxWarning(QObject::tr("窗口正在轮询视频!"));
		return -1;
	}
	...
	if (!bThreadExit)
	{
		//QMessageBox::warning(NULL, QObject::tr("警告"), QObject::tr("视频轮询正在结束，请稍后再试"));
	...
	TabChannelInfo_t* pChennel = pDev->DevInfo->GetChannelInfo(pDev->Channel);
	if (pChennel == NULL)
	{
		mlog(LOG_ERR, "AddWatch 获取通道信息出错 id:%d channel:%d!\n", pDev->DevInfo->GetDevInfo()->nDevId, pDev->Channel);
		return  -1;
	}
	strName = QString("#%1 %2 ").arg(m_ndspIndex + 1).arg(pChennel->szChannelName);
	strcpy(m_dspWndinfo.szDevName, pChennel->szChannelName);
	m_dspWndinfo.nDevId = pChennel->SlaveDevId;
	m_dspWndinfo.bVideoStop = false;
	//ipPortLabel->setText(QString("%1").arg(strName));
	...
	hThread = CreateThread(NULL, 0, RealPlayThread, this, 0, &dwThreadId);
	if (hThread)
	{
		CloseHandle(hThread);
		hThread = NULL;
...
DWORD WINAPI OneVideoPro::RealPlayThread(LPVOID lpParam)
{
	OneVideoPro* pDevice = (OneVideoPro*)lpParam;
	if (NULL == pDevice)
	{
		return -1;
	}
...
void OneVideoPro::StopWatch()
{
	...
	Speak(FALSE);
	//Patrol(FALSE);
	//PresetPatrol(FALSE, -1);
	
	m_bAudioEnable = TRUE;
	if (m_pDev != NULL)
	{
		m_pDev->DevInfo->CloseChannel((HWND)ui.label->winId());
	}
	m_dspWndinfo.bVideo = FALSE;
...
void OneVideoPro::StartWatch()
{
	...
	if (m_pDevTemp->DevInfo->OpenChannel(m_pDevTemp->Channel, nStream, (HWND)ui.label->winId(), m_nWndNum) < 0)
	{
		if (m_bTipBox)
		{
	...
	custevent->setId(2);
	QApplication::postEvent(this, custevent);
	IPLabel->setText(QString("IP:%1").arg(m_pDevTemp->DevInfo->GetDevInfo()->szIpAddr));
...
void OneVideoPro::ManulRecord()
{
	...
	int nChannel = m_pDev->DevInfo->GetPlayChannel((HWND)ui.label->winId());
	if (m_dspWndinfo.bManulRecord)
	{
		if (m_pDev->DevInfo->ManulRecord((HWND)ui.label->winId(), nChannel, TRUE) == 0)
		{
			m_dspWndinfo.bManulRecord = FALSE;
		}
	}
	...
	{
		if (m_pDev->DevInfo->ManulRecord((HWND)ui.label->winId(), nChannel, FALSE) == 0)
		{
...
int OneVideoPro::OpenSound()
{
	...
	if (m_dspWndinfo.bSound)
	{
		if (m_pDev->DevInfo->EnableAudio((HWND)ui.label->winId(), FALSE) == 0)
		{
			m_dspWndinfo.bSound = FALSE;
			soundBtn->setIcon(QIcon(":/res/SoftIcon/voiceClose.png"));
			return 0;
	...
	{
		if (m_pDev->DevInfo->EnableAudio((HWND)ui.label->winId(), TRUE) == 0)
		{
			m_dspWndinfo.bSound = TRUE;
			soundBtn->setIcon(QIcon(":/res/SoftIcon/voiceOpen.png"));
			return 1;
		}
	}
	return 0;
...
int OneVideoPro::LocalRecord()
{
	...
	if (m_dspWndinfo.bLocRecord)
	{
		if (m_pDev->DevInfo->LocalRecord((HWND)ui.label->winId(), szRecFilePath, TRUE) == 0)
		{
	...
	else
	{
		...
		if (set->g_ClientCfg.nCycleRecord)
		{
		while (sDir.isEmpty())
		{
			clearFiles(sDir);
		...
		if (m_pDev->DevInfo->LocalRecord((HWND)ui.label->winId(), szRecFilePath, FALSE) == 0)
		{
			m_dspWndinfo.bLocRecord = TRUE;
			recordBtn->setIcon(QIcon(":/res/SoftIcon/VideoOpen.png"));
			RECORD_TIMER = set->g_ClientCfg.nFileTimeLen * 1000;
			CHKDISK_TIMER = 15000;
			return 1;
		}
		else
...
QString OneVideoPro::GetAvailDisk(unsigned long nMinAvailBytes)
{
	...
	for (i = set->g_ClientCfg.nBeginDriver; i <= set->g_ClientCfg.nEndDriver; i++)
	{
...
int OneVideoPro::Speak(BOOL bStart)
{
	...
	if (!bStart)
	{
		...
		{
			if (m_pDev->DevInfo->Speak(m_hWnd, bStart) == 0)
			{
				m_dspWndinfo.bSpeak = FALSE;
				talkBtn->setIcon(QIcon(":/res/SoftIcon/talkClose.png"));
	...
	else
	{
		if (!m_dspWndinfo.bSpeak)
		{
			if (m_pDev->DevInfo->Speak(m_hWnd, bStart) == 0)
			{
				...
				talkBtn->setIcon(QIcon(":/res/SoftIcon/talkOpen.png"));
			}
		}
	}
...
void OneVideoPro::OnMenuStreamMain()
{
	...
	if (m_dspWndinfo.bMainStream)
		return;
	m_pDev->DevInfo->ChangeChannelStream((HWND)ui.label->winId(), DEVICE_MAIN_STREAM);
	m_dspWndinfo.bMainStream = TRUE;
}
...
void OneVideoPro::OnMenuStreamSub()
{
	...
	if (!m_dspWndinfo.bMainStream)
		return;
	m_pDev->DevInfo->ChangeChannelStream((HWND)ui.label->winId(), DEVICE_SUB_STREAM);
...
void OneVideoPro::CapturePic()
{
	...
	sprintf(szPicFilePath,"%s\\%s_%s.bmp", set->g_ClientCfg.szPicPath, szFile.toLatin1().data(), str.toLatin1().data());
	if (m_pDev->DevInfo->CapturePic((HWND)ui.label->winId(), szPicFilePath) < 0)
...
bool OneVideoPro::event(QEvent *e)
{
	...
	if (e->type() == static_cast <QEvent::Type>(CompareEvent::getType(CET_MESSAGEBOX)))
	{
