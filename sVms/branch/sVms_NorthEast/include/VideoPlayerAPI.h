...
typedef void (CALLBACK *CoverFileCallBack) (DWORD CurrentPos, DWORD TotoalPos,DWORD dwUser);//转换进度

//信息帧回调
typedef void (CALLBACK *InfoFrameCallBack)(LONG nPort, LONG nType, LPCSTR pBuf,LONG nSize, long nUser);

//对讲
//typedef int (WINAPI *TalkCaptureData)(BYTE *pBuffer, int nBufLen, DWORD dwContext, void *pContext);
//以下为对外接口，供用户作二次开发调用
...
//VIDEO_PLAYER_API BOOL CALLMETHOD VIdeoPlayer_StopConverFile(LONG lConvertHand);

//信息帧回调
VIDEO_PLAYER_API BOOL 	CALLMETHOD 	VideoPlayer_SetInfoFrameCallBack(LONG nPort, InfoFrameCallBack pProc, LONG nUser);
